...
DEFAULT_OUT_DIR = os.path.join(SPECTRA_FOLDER, "gaussian_fit_results")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Automatically fit recurring spectral lines in one or more SDSS FITS "
                    "spectra, choosing the best of Gaussian, sigmoid, or Voigt profiles per line."
    )
    ...
    parser.add_argument("--out-dir", type=str, default=DEFAULT_OUT_DIR,
                        help="Directory to write per-file '<fit_type>_<filename>.csv' results into "
    ...
    parser.add_argument("--verbose", action="store_true",
                        help="Print the R^2 (and width) of the fit for every line.")
...
def main():
    ...
    print(f"Results saved to: {args.out_dir}/<fit_type>_<filename>.csv (one CSV per input file)")
    if plots_dir:
